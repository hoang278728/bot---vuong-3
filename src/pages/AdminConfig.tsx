...
const AdminConfigLayout: React.FC<{
	darkMode: boolean;
	toggleDarkMode: () => void;
	handleLogout: () => void;
	children: React.ReactNode;
}> = ({ darkMode, toggleDarkMode, handleLogout, children }) => {
	return (
		<div
...
const TabSelector: React.FC<{
	...
	return (
		<div className='mb-8 flex justify-center'>
			...
			>
				{['settings', 'telegram', 'router'].map((tab) => (
					<button
						key={tab}
...
}> = ({ config, activeTab, darkMode, handleChange, handleCheckboxChange, handleSubmit }) => {
					...
					<div className='space-y-4'>
						<div className='flex flex-col space-y-2'>
							<label
								className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'} flex items-center`}
							>
								<IconLink className='mr-2 h-5 w-5 text-blue-500' />
								<span>Business URL</span>
							</label>
							<input
								type='text'
								name='router.business_url'
								value={config.router.business_url}
								onChange={handleChange}
								className={`mt-1 block w-full rounded-md border ${
									darkMode ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white'
								} px-3 py-2 text-sm placeholder-gray-400 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500`}
							/>
						</div>
					</div>
				</div>
...
const AdminConfig: React.FC = () => {
	...
	useEffect(() => {
		fetchConfig(setConfig, setError, navigate);
	}, [navigate]);

	const handleChange = useConfigChange(setConfig);
	const handleCheckboxChange = useCheckboxChange(setConfig);
	const handleSubmit = useConfigSubmit(config, setSuccess, setError);
	...
	return (
		<AdminConfigLayout darkMode={darkMode} toggleDarkMode={() => setDarkMode(!darkMode)} handleLogout={handleLogout}>
			...
			<SupportContact darkMode={darkMode} />
		</AdminConfigLayout>
	);
};
